...
namespace Piglet
{
    ...
    public class ZipUtil
    {
            ...
            using (var stream = File.OpenRead(zipPath))
            {
                using (var archive = new ZipFile(stream))
                {
                    ...
                    foreach (var result in
                        GetEntry(archive, new Regex("\\.(gltf|glb)$")))
                    {
                        entry = result;
                    }
        ...
        /// </summary>
        static public IEnumerable<byte[]> GetEntryBytes(ZipFile archive, Regex regex)
        {
            bool matcher(ZipEntry candidate) => regex.IsMatch(candidate.Name);

            byte[] data = null;
            foreach (var result in GetEntryBytes(archive, matcher))
            {
                data = result;
                yield return null;
            }
        ...
        /// </summary>
        static public IEnumerable<byte[]> GetEntryBytes(ZipFile archive, string filename)
        {
        ...
        /// </summary>
        static public IEnumerable<byte[]> GetEntryBytes(
            ZipFile archive, Func<ZipEntry, bool> matcher)
        {
            ZipEntry entry = null;
